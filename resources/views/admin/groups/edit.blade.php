                    ...
                    @if($role)
                        <form class="form-horizontal" role="form" method="post" action="">
                            ...
                            <div class="form-group">
                             ...
                             @if(!is_null($permissions) and !is_null($role->permissions))
                            <div class="form-group {{ $errors->first('permissions', 'has-error') }}">
        
                                <label for="title" class="col-sm-2 control-label">
                                    @lang('groups/form.permissions')
                                </label>
                                <div class="col-sm-5">
                                    <ul class="admin-permissions-design">
                                        
                                    
                                    @foreach($permissions as $key => $value)
                                    <li>
                                    
                                        @if(isset($role->permissions[$key]) && $key == $role->permissions[$key])
                                            {!! Form::checkbox('permissions[]', $key,true) !!} : {{ $value }} 
                                        @else
                                            {!! Form::checkbox('permissions[]', $key) !!} : {{ $value }} 
                                        @endIf
                                        </li>
                                    @endForeach
                                    </ul>
